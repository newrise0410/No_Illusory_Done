...
- Stage A: `--ci` runs the frozen `SETUP:` lines itself (clean env), then the gates. Fail → `CI: reject`, stop. Each rejected `--ci` increments `refs/nid/ci_attempts`; at `max_ci_attempts` the verdict is a HANDOFF.
